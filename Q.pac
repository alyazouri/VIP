// ============================================================================
// ⭐ FLAVORLESS MASTERPIECE PAC SCRIPT — VIP JORDANIAN COMMUNITY EDITION ⭐
// [ 100% JORDAN MATCHMAKING | SMART SESSION RESET | VIVOX VOICE | 0% LOADING ]
// ============================================================================
...
// ================= 2. PURE JORDAN NETWORKS (IPV4) — نطاقات أردنية بحتة 100% =================
var JORDAN_MATCH_IPV4 = [
  // ── Tier 1 Main Jordanian Telecoms (مزودو الخدمة المحليون في عمّان) ──
  ...
  ["213.6.0.0",     "255.255.0.0"],     // /16 — Jordan Telecom Group / Paltel
...
function FindProxyForURL(url, host) {
  ...
  if (isIPv6(ip)) {
    if (isMatch(url, host)) {
      ...
      // ⏱️ نظام إعادة ضبط الجلسة الذكي (تفريغ تلقائي بعد 45 دقيقة للمباريات الجديدة)
    ...
    // 🎪 توجيه وظائف اللوبي والشات عبر البروكسي الأردني بسلاسة
  ...
  // 🎮 قفل المباريات على خوادم الأردن المحلية 100% (أقل بينغ ممكن)
  if (isMatch(url, host)) {
  ...
  // (يمنع تعليق اللوبي ويظهر لاعبين أردنيين فقط في التجنيد والشات)
  if (isLobby(url, host) || isSocial(url, host) || isStore(url, host) || isCDN(url, host)) {
    return pickLobbyProxy(host);
  }
