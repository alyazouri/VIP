// ======================================================
// PAC – PUBG Mobile الأردن | Full Mode Coverage v2
// UDP SOCKS5 مُفعَّل بالكامل – جميع البورتات مفتوحة
// Lobby / Match (UDP) / CDN / Voice – مسارات مستقلة
// تحديث: 28/03/2026
// ======================================================
...
// ══════════════════════════════════════════════════════
// تعريف البورتات لكل مسار
// ══════════════════════════════════════════════════════
var PORTS = {

  // اللوبي: Auth / Matchmaking / Store / Friends / Account
  lobby: {
    socks: [20000, 20001, 20002],
    http:  [8080, 8081]
  },

  // المباريات: UDP حي لجميع مودات اللعبة
  // Classic | TDM | Payload | Metro Royale | Arena
  // EvoGround | Survive Till Dawn | Bluehole | Infection
  match: {
    socks: [40000, 40001],   // ← UDP-first، مُفعَّل بالكامل
    http:  [8085, 8087, 8088, 8880]
  },

  // CDN والتحديثات: تحميل الباتشات والأصول والخرائط
  cdn: {
    socks: [8000, 8001],
    http:  [8080, 8081]
  },

  // الصوت: Voice Chat داخل المباريات والغرف
  voice: {
    socks: [30000],
    http:  [8080]
  }
...
// اللوبي – API / Auth / Matchmaking / Store
var LOBBY_DOMAINS = [
  "intlgame.com",
  "msdkglobal.qq.com",
  "sgp-game.qq.com",
...
// المباريات الحية – سيرفرات اللعبة لكل المودات
var MATCH_DOMAINS = [
  "proximabeta.com",
  "proximabeta.net",
  "tencentyun.com",
...
// ══════════════════════════════════════════════════════
// نطاقات IPv6 الأردنية – أول خانتان ثابتتان
// ══════════════════════════════════════════════════════
var JO_IPV6 = [
...
function FindProxyForURL(url, host) {
  ...
  // 2. الصوت – أعلى أولوية لضمان جودة الـ Voice Chat
  if (hostInList(host, VOICE_DOMAINS)) return voiceChain();

  // 3. CDN والتحديثات
  if (hostInList(host, CDN_DOMAINS)) return cdnChain();

  // 4. اللوبي: تسجيل دخول / مطابقة / متجر / حساب
  if (hostInList(host, LOBBY_DOMAINS) || hostInList(host, LOCAL_DOMAINS))
    return lobbyChain();

  // 5. المباريات الحية – UDP عبر SOCKS5 40000/40001
  if (hostInList(host, MATCH_DOMAINS)) return matchChain();

  // 6. عناوين IP أردنية مباشرة → مسار المباريات (UDP)
  if (isIPAddress(host) && (isJoIPv4(host) || isJoIPv6(host)))
    return matchChain();

  // 7. أي دومين يحمل كلمة مفتاحية → مسار اللوبي كـ fallback آمن
  if (hasKeyword(host) || hasKeyword(url)) return lobbyChain();

  // 8. FORCE_ALL – كل حركة غير مُصنَّفة عبر اللوبي
  return lobbyChain();
}
