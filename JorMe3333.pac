...
// GLOBAL JO PRESSURE (ALL MODES) + AF (Afghanistan) HARD BLOCK
...
function isAfghanistanIP(ip){
  ...
  var maybe =
    ...
    ip.indexOf("43.250.")===0 || ip.indexOf("64.224.")===0;
  ...
  // extra AF
  if (isInNet(ip, "43.250.136.0", "255.255.252.0")) return true; // /22
  if (isInNet(ip, "64.224.144.0", "255.255.240.0")) return true; // /20

  // 103.* AF clusters (tight)
  if (isInNet(ip, "103.5.172.0",  "255.255.252.0")) return true;
  if (isInNet(ip, "103.13.64.0",  "255.255.252.0")) return true;
  if (isInNet(ip, "103.15.238.0", "255.255.254.0")) return true;
  ...
  if (isInNet(ip, "103.30.136.0", "255.255.252.0")) return true;
  if (isInNet(ip, "103.42.0.0",   "255.255.252.0")) return true;
  if (isInNet(ip, "103.46.208.0", "255.255.252.0")) return true;

  // 45.* tight
  if (isInNet(ip, "45.65.56.0",   "255.255.252.0")) return true;
  if (isInNet(ip, "45.116.128.0", "255.255.254.0")) return true;
  if (isInNet(ip, "45.126.253.0", "255.255.255.0")) return true;
...
// GLOBAL: Jordan only first across ALL MODES
var GLOBAL_JO_ONLY_MS = 240000; // 4 minutes
var GLOBAL_START_TS = Date.now();
function globalJOOnly(){
  return (Date.now() - GLOBAL_START_TS) < GLOBAL_JO_ONLY_MS;
}

var RECRUIT_JO_ONLY_MS = 90000; // 90s
var RECRUIT_START_TS = Date.now();
function recruitJOOnly(){
  return (Date.now() - RECRUIT_START_TS) < RECRUIT_JO_ONLY_MS;
}

var ARENA_JO_ONLY_MS = 45000;      // 45s
var ARENA_GULF_ONLY_MS = 180000;   // then JO/Gulf
