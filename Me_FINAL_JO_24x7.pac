...
function _buildHyperChain(strategy, isJO, isNeighbor, ultraMode) {
  ...
  // Geographic intelligence (NO DIRECT): keep routing inside proxy chains only
  // NOTE: DIRECT fallback removed to avoid path escaping outside JO chain.
...
function FindProxyForURL(url, host) {
  ...
  // Priority 1: CDN traffic (keep inside JO chain — NO DIRECT)
  if (traffic.strategy === "CDN_TURBO") {
    return _buildHyperChain("CDN_TURBO", isJO, isNeighbor, ultraMode);
  }
  ...
  // Priority 6: Jordan IPs (JO BIAS STRONG)
  if (isJO) {
    // Strong JO bias: treat JO-resolved targets as critical matchmaking to stay local
    return _buildHyperChain("HYPER_MATCHMAKING", true, false, ultraMode);
  }

  // Priority 7: Neighbor Countries (دول الجوار) — keep inside JO chain (NO DIRECT)
  if (isNeighbor) {
    // Neighbor fallback through JO-balanced chain (helps keep region close to JO)
    return _buildHyperChain("BALANCED_FAST", isJO, true, ultraMode);
  }

  // Priority 8: Low Priority / Analytics — still keep inside proxy chain (NO DIRECT)
  if (traffic.priority < 30) {
    return _buildHyperChain("BALANCED_FAST", isJO, isNeighbor, ultraMode);
  }

  // ═══════════════ DEFAULT: ALWAYS-ON PROXY ROUTING (NO DIRECT) ═══════════════
  return _buildHyperChain("BALANCED_FAST", isJO, isNeighbor, ultraMode);
}
